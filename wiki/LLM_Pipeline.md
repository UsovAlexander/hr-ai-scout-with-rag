...
- **Произвольная вакансия:** `evaluate_custom_vacancy(vacancy_text, resume_id)` —
  тот же пайплайн, но текст вакансии передаётся напрямую (для введённых в UI
  вакансий). Ядро вынесено в `evaluate_with_texts(...)`.
